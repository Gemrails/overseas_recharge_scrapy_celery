...
class CRedis(object):
    ...
    def __init__(self, portconf='redis_cnf'):
        self.cf = Config(confpath='config.ini')
        self.log = LogSys()
        #self.host = 'localhost'
        #self.port = 6379
        #self.db = 0
        self.host = self.cf.getvalue('db-info', 'host')
        self.port = self.cf.getvalue('db-info', 'port')
        self.db = self.cf.getvalue('db-info', 'db')
        try:
            self.r = redis.Redis(host = self.host, port = self.port, db = self.db)
        except Exception, e:
            errmsg = "CRedis connect redis Err.Class CRedis." + str(e)
    ...
    # 操作list 2014-04-13
    def llrange(self, key, start, end):
    ...
    def pipline(self, key, value):
        return self.r.pipeline().rpush(key, value)
